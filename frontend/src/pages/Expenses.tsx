...
import { expensesAPI, seasonsAPI, teamsAPI } from '../services/api';
...
export default function Expenses() {
  ...
  const { data: expenses = [] } = useQuery({
    queryKey: ['expenses', selectedSeason, selectedTeam],
    queryFn: () => expensesAPI.getAll(selectedSeason || undefined, selectedTeam || undefined),
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-text-secondary">Track all expenses for your sports organization</p>
          </div>
          <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 rounded-lg bg-bg-primary border border-white/10 text-white focus:outline-none focus:border-sports-primary"
                  >
                    <option value="">Season-wide expense</option>
                    {teams
            ...
            >
              <div className="flex-1">
                ...
                <div className="space-y-1 text-sm text-text-secondary">
                  <p>
                    {expense.team_id
                      ? teams.find((t) => t.id === expense.team_id)?.name || 'Team'
                      : seasons.find((s) => s.id === expense.season_id)?.name || 'Season'}
                  </p>
                  <p>Date: {format(new Date(expense.payment_date), 'MMM d, yyyy')}</p>
                  {expense.vendor && <p>Vendor: {expense.vendor}</p>}
                  {expense.receipt_number && <p>Receipt: {expense.receipt_number}</p>}
                  {expense.notes && <p className="mt-2">{expense.notes}</p>}
                </div>
              </div>
        ...
        {expenses.length === 0 && (
          <div className="bg-bg-secondary rounded-lg p-12 text-center border border-white/10">
            <TrendingDown className="w-16 h-16 text-text-secondary mx-auto mb-4" />
